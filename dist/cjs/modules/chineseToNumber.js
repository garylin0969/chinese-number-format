...
function chineseToNumber(str) {
    ...
    const unitValue = constants_1.UNIT_MAP.get(lastChar);
    ...
    // 2. 分離整數與小數部分
    let pointChars = [...constants_1.CONFIGS.base['.']];
    ...
    for (const char of intStr) {
        if (constants_1.UNIT_MAP.has(char)) {
            hasUnit = true;
            break;
        }
    }
    if (!hasUnit) {
        ...
        let resultStr = '';
        for (const char of intStr) {
            const n = constants_1.CHINESE_TO_NUMBER_MAP.get(char);
            if (n !== undefined) {
                resultStr += n;
            }
        }
    ...
    for (let i = 0; i < intStr.length; i++) {
        const char = intStr[i];
        const num = constants_1.CHINESE_TO_NUMBER_MAP.get(char);
        if (num !== undefined) {
            if (num === 0) {
                hasZero = true; // 標記零出現
                continue;
            ...
            const unit = constants_1.UNIT_MAP.get(char);
            if (unit !== undefined) {
                lastUnit = unit;
...
exports.default = chineseToNumber;
